...
export default function Agriculture() {
  ...
  const fetchData = async () => {
    try {
      const filter = where("typeCode", "array-contains", "HEALTH");
      let q = query(collection(db, "companies"), filter);
      const data: any = [];
  ...
  return (
    <div>
      ...
      {loading ? <Loading /> : (<DataTable
        title={<span style={{ fontFamily: "Cascadia Mono" }}>Agriculture insurance</span>}
        columns={columns}
        data={hData}
        pagination
        fixedHeader
